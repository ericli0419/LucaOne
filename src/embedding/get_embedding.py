...
try:
    ...
    from utils import set_seed, to_device, get_labels, get_parameter_number, gene_seq_replace, \
        download_trained_checkpoint_lucaone, clean_seq_luca, available_gpu_id, matrix_2_vector, \
...
def predict_embedding(
    ...
    if isinstance(llm_dirpath, str):
        ...
        for item in llm_dirpath.items():
            ...
            global_args_info[key]["use_bf16"] = use_bf16
    ...
    if isinstance(llm_dirpath, str):
        ...
        if seq_type == "gene":
            ...
            raise ValueError("Not support seq_type=%s for LucaOne" % seq_type)
    ...
    if model_args.input_file and os.path.exists(model_args.input_file):
        ...
        while True:
            ...
            if use_cpu:
                print("use_cpu: %r" % use_cpu)
            if emb is not None:
                print("processed seq length: %d, truncation seq length: %d" % (emb.shap[0], truncation_seq_length))
                break
            print("%s embedding error, max_len from %d truncate to %d" % (
                model_args.seq_id,
                truncation_seq_length,
